<?php
    ...
    $sql = "SELECT sum(ProductionQty) as tot from fakta_produksi";
    $tot = mysqli_query($mysqli,$sql);
    $tot_order = mysqli_fetch_row($tot);

    //echo $tot_amount[0];

    //query untuk ambil pembelian berdasarkan vendor, query sudah dimodifikasi
    //ditambahkan label variabel DATA. (teknik gak jelas :D)

	$sql = "SELECT concat('name:',l.Location_Name) as name, concat('y:', sum(fp.ProductionQty)*100/" . $tot_order[0] .") as y, concat('drilldown:', l.Location_Name) as drilldown
            FROM location l
            JOIN fakta_produksi fp ON (l.Location_ID = fp.Location_ID)
            GROUP BY name
            ORDER BY y DESC"   ;         
            //echo $sql;
    ...
    $sql = "SELECT l.Location_Name lokasi, sum(fp.ProductionQty) as tot_prd
    ...
    $sql = "SELECT l.Location_Name lokasi, 
            t.bulan as bulan, 
            sum(fp.ProductionQty) as pendapatan_kat
            FROM location l
            JOIN fakta_produksi fp ON (l.Location_ID = fp.Location_ID)
            JOIN time t ON (t.time_id = fp.Time_ID)
            GROUP BY lokasi, bulan";
...
Highcharts.chart('container', {
    ...
    title: {
        text: 'Persentase Jumlah Produksi - Semua Lokasi'
    },
    subtitle: {
        text: 'Klik di potongan kue untuk melihat detil jumlah produksi disetiap lokasi berdasarkan bulan'
    },
